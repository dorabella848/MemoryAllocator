...
static void Malloc_Complexity(benchmark::State& state) {
    auto alloc = std::make_unique<Allocator<int>>();
    ...
    for (auto _ : state) {
        ...
        if (alloc->getFreeMemory() < state.range(0)) {
            alloc.reset();
            alloc = std::make_unique<Allocator<int>>();
        }
        state.ResumeTiming();
    }
...
static void Malloc_Performance(benchmark::State& state){
    auto alloc = std::make_unique<Allocator<int>>(INT32_MAX);
    ...
    for (auto _ : state){
        ...
        if (ptr == nullptr) {
            state.PauseTiming();
            alloc.reset();
            alloc = std::make_unique<Allocator<int>>(INT32_MAX);
            state.ResumeTiming();
        }
    }
...
static void ReallocMore_Complexity(benchmark::State& state){
    Allocator<int> alloc(INT32_MAX);
    int* TestChunk = alloc.allocate(state.range(0));
